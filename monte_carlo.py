...
if st.sidebar.button("🚀 開始模擬運算", type="primary", use_container_width=True):
    with st.spinner('⚙️ 運算中...'):
        ...
        # 其他原有策略 (1, 2, 3, 5)
        V1, V2_c, V2_L, V3_c, V3_L, V5_B, V5_L = [np.ones(N) * initial_capital if i < 1 else np.zeros(N) for i in [1, 0.5, 0.5, 0.5, 0.5, 1, 0]]
        V1 = np.ones(N) * initial_capital
        V2_c, V2_L = np.ones(N)*initial_capital*0.5, np.ones(N)*initial_capital*0.5
        V3_c, V3_L = np.ones(N)*initial_capital*0.5, np.ones(N)*initial_capital*0.5
        ...
        for d in range(1, total_days + 1):
            ...
            # --- 原有策略計算 ---
            V1 *= r_L
            V2_c *= cash_growth; V2_L *= r_L
            V3_c *= cash_growth; V3_L *= r_L
    ...
    for col in df_res.columns:
        data = df_res[col]
        stats.append({'投資策略': col, '獲勝機率 (%)': f"{(data > initial_capital).mean()*100:.2f}%", '平均終值': f"{data.mean():,.0f}", '中位數': f"{data.median():,.0f}", '悲觀 5%': f"{np.percentile(data, 5):,.0f}"})
