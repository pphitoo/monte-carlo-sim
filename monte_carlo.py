...
st.title("🔬 蒙地卡羅量化回測實驗室 (雙 API 實戰版)")
st.markdown("完美結合**「初期單筆資金」**與**「自訂次數的分期資金」**。支援雙資料庫交叉比對，真實還原你的資金流在各種平行宇宙中的勝率與極端風險。")
...
with st.expander("📖 實驗室說明與 6 大情境策略 (點擊展開)", expanded=False):
    ...
    * **策略 4. 紀律經理 (50/50 再平衡)**：跟策略 3 一樣一半股票一半存款，但每年底會「重新平衡」，把賺錢的部分賣掉存起來，或拿存款補虧損的股票。
...
if "歷史" in engine:
    ...
    api_source = st.sidebar.radio("📡 歷史資料庫來源", [
        "1. Yahoo Finance (全球適用/內建極端值濾網)", 
        "2. FinMind API (台股專用/官方極精準數據)"
    ])
    ...
    if "Yahoo" in source:
        try:
            ...
            if isinstance(data.columns, pd.MultiIndex):
                rets = data['Close'].iloc[:, 0].pct_change().dropna().values
            else:
                rets = data['Close'].pct_change().dropna().values
            
            # 🌟 Yahoo 濾水器：把絕對值 > 15% 的離譜漲跌幅 (如那顆 -75% 核彈) 設為 0，防止污染樣本
...
if st.sidebar.button("🚀 開始實戰模擬", type="primary", use_container_width=True):
    with st.spinner(f'⚙️ 正在根據你的彈性資金流進行平行宇宙運算...'):
        ...
        if "歷史" in engine:
            rets = get_hist_data(ticker, start_date, end_date, api_source)
            if rets is None or len(rets) < block_size:
                st.error("❌ 無法載入歷史資料。如果使用 FinMind，請確保代碼是台股 (如 0050.TW 或 0050 皆可)。")
                st.stop()
        ...
        v4_c = np.ones(N) * initial_cap * 0.5; v4_b = np.ones(N) * initial_cap * 0.5
        ...
        for d in range(days):
            ...
            v4_c *= cash_growth; v4_b *= rb
            if (d+1)%252==0: v4_c, v4_b = (v4_c+v4_b)*0.5, (v4_c+v4_b)*0.5
            ...
            if d % dca_interval == 0 and (d // dca_interval) < dca_parts:
                ...
                v4_c += periodic_cap * 0.5; v4_b += periodic_cap * 0.5
                v5_b += periodic_cap 
    ...
    # 5. 產出報表
    # ==========================================
    st.success(f"✅ 成功完成 {sim_years} 年蒙地卡羅模擬 ({api_source.split(' ')[1]})！總投入成本：{total_capital_wan:.1f} 萬。")
    ...
    title_prefix = f"Historical Block ({api_source.split(' ')[1]})" if "歷史" in engine else "GBM Fat-Tail"
    ...
    st.info("👈 請在左側選擇你要使用的「歷史資料庫來源」，準備開始運算。")
