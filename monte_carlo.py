...
dca_parts = st.sidebar.slider(
    "分批買入次數 (次)", min_value=2, max_value=48, value=12)
dca_interval = st.sidebar.slider(
    "買入頻率 (隔多少交易日買一次)", min_value=5, max_value=60, value=21, help="21天約為一個月")
st.sidebar.divider()
...
if "歷史" in engine:
    ...
    max_date = today  # 我們前面已經定義 today = datetime.now().date()
...
drag_annual = st.sidebar.slider(
    "槓桿標的 額外耗損 (%)", min_value=0.0, max_value=10.0, value=1.5, step=0.1) / 100

st.sidebar.caption("策略 5：跌深抄底設定")
drop_threshold = st.sidebar.slider(
    "回檔觸發比例 (%)", min_value=5, max_value=50, value=20) / 100
transfer_pct = st.sidebar.slider(
    "轉入比例 (%)", min_value=10, max_value=100, value=20) / 100
...
if st.sidebar.button("🚀 開始模擬運算", type="primary", use_container_width=True):
    with st.spinner('⚙️ 運算中...'):
        ...
        if "歷史" in engine:
            ...
            Z = np.clip(np.random.standard_t(df=df_t, size=(
                total_days, N)) * np.sqrt(1/3), -25, 25)
            log_ret_base = (mu_base - 0.5 * sig_base**2) * \
                dt + sig_base * np.sqrt(dt) * Z
            log_ret_lev = (mu_base*lev_mult - 0.5*(sig_base*lev_mult)**2) * \
                dt + (sig_base*lev_mult)*np.sqrt(dt)*Z - daily_drag
            sim_ret_base, sim_ret_lev = np.exp(
                log_ret_base) - 1, np.exp(log_ret_lev) - 1
        ...
        # 初始化策略
        V4_LS = np.ones(N) * initial_capital  # 4. 一次買入基準標的
        V6_DCA_cash = np.ones(N) * initial_capital  # 6. 分批買入基準標的 - 現金池
        V6_DCA_stock = np.zeros(N)                 # 6. 分批買入基準標的 - 股票池

        # 其他原有策略 (1, 2, 3, 5)
        V1, V2_c, V2_L, V3_c, V3_L, V5_B, V5_L = [np.ones(
            N) * initial_capital if i < 1 else np.zeros(N) for i in [1, 0.5, 0.5, 0.5, 0.5, 1, 0]]
        ...
        for d in range(1, total_days + 1):
            ...
            # 策略 6: 分批投入
            V6_DCA_cash *= cash_growth
            V6_DCA_stock *= r_B
            # 判斷是否執行買入
            current_step = (d - 1) // dca_interval
            if (d - 1) % dca_interval == 0 and current_step < dca_parts:
    ...
    ax.axvline(initial_capital, color='red',
