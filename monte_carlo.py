...
st.markdown(
    "自由切換「歷史區塊抽樣」與「GBM 肥尾數學模型」，分析「一次投入」與「分批買入」在不同環境下的勝率。數據已自動換算為**「萬 (10k TWD)」**單位。")
...
# 🌟 讓介面以「百萬」為單位，例如輸入 2.0 代表 200萬
input_mb = st.sidebar.number_input(
    "初始資金 (百萬)", min_value=0.1, value=2.0, step=0.1)
...
dca_interval = st.sidebar.slider(
    "買入頻率 (交易日)", min_value=5, max_value=60, value=21)

if "歷史" in engine:
    ticker = st.sidebar.text_input("輸入代碼 (Yahoo Finance)", value="0050.TW")
    col1, col2 = st.sidebar.columns(2)
    start_date = col1.date_input("開始", value=datetime(
        2008, 1, 1).date(), min_value=min_date, max_value=today)
    end_date = col2.date_input(
        "結束", value=today, min_value=min_date, max_value=today)
...
if st.sidebar.button("🚀 開始模擬運算", type="primary", use_container_width=True):
    with st.spinner('⚙️ 正在建構平行宇宙與運算策略...'):
        ...
        if "歷史" in engine:
            ...
            indices = np.random.randint(
                0, len(rets)-block_size, (int(np.ceil(days/block_size)), N))
            ...
            Z = np.clip(np.random.standard_t(
                df_t, (days, N)) * np.sqrt(1/3), -15, 15)
            # 使用對數報酬近似
        ...
        v1, v2_c, v2_L, v3_c, v3_L, v4, v5_B, v5_L, v6_c, v6_s = [
            np.ones(N)*initial_capital for _ in range(10)]
        ...
        for d in range(days):
            ...
            v3_L *= rb
            if (d+1) % 252 == 0:
                v3_c, v3_L = (v3_c+v3_L)*0.5, (v3_c+v3_L)*0.5
            # 策略 5: 跌深抄底
            v5_B *= rb
            v5_L *= rl
    ...
    for col in df_res_van.columns:
        sns.kdeplot(df_res_van[col], ax=ax, label=col,
                    fill=True, alpha=0.15, linewidth=2)

    ax.axvline(initial_capital_van, color='red', linestyle='--',
    ...
    ax.set_title(
        f'{sim_years}-Year Final Asset Distribution ({title_prefix} / 10k TWD)', fontsize=14)
